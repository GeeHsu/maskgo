    ...
    <div class="col-sm-3">
      <div v-if="cityName.length" class="toolbox">
        <div class="sticky-top bg-white shadow-sm p-2">
          ...
          <p class="mb-0 small text-muted text-right">請先選擇區域查看（點選資料地圖定位）</p>
        </div>
        ...
        <ul class="list-group">
          <template v-for="(item, key) in data">
            <a class="list-group-item text-left" :key="key"
              ...
              <p class="mb-0">備註: {{ item.properties.note }}</p>
            </a>
          </template>
        </ul>
      </div>
...
const osm = {
  addMapMarker(x, y, item) {
    const icon = item.mask_adult || item.mask_child ? icons.blue : icons.red;
    L.marker([y, x], {
      icon,
    }).addTo(osmMap).bindPopup(`<strong>${item.name}</strong> <br>
    口罩剩餘：<strong>成人 - ${item.mask_adult ? `${item.mask_adult} 個` : '目前無庫存'}/ 兒童 - ${item.mask_child ? `${item.mask_child} 個` : '目前無庫存'}</strong><br>
